...
* @version $Id: image_captcha.php 39579 2014-03-17 10:29:30Z weinert $
*/
...
class image_captcha extends base_dynamicimage {
  ...
  */
   ...
   * @var array
   */
  private $textPos = array('x' => 0, 'y' => 0);

  /**
   * @var array
   */
  private $captchaChars = array();
  ...
  * @access private
  * @return resource $result resource id
  */
  function generateImage($controller) {
    ...
    $imageFinal = $this->imageFilterWaves($imageFinal);
  ...
  */
  ...
  function _generateImageString() {
    ...
    if ($fontType = $mediaDB->getFileName($this->data['font_type'])) {
      ...
      foreach ($this->captchaChars as $data) {
        imagettftext(
          $result,
          $fontSize,
          $data['angle'],
  ...
  * @return resource
  */
  function imageFilterWaves($srcImage) {
